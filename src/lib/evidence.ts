...
//
//  Integrates with lib/trust.ts (TrustClassifier).
// ============================================================

import { TrustClassifier, TRUST_LEVELS, CONTENT_SOURCES, TrustRecord, TrustLevel, ContentSource } from './trust';
...
export type ClaimOrigin = typeof CLAIM_ORIGIN[keyof typeof CLAIM_ORIGIN];
...
export interface NoteValidationResult {
    ...
    blocked: number;
    allowed: number;
    issues: Array<{ index: number; text: string; reason: string }>;
    summary: { allowed: number; blocked: number; origins: Record<string, number>; trust: Record<string, number> };
}
...
export function isUrlLike(text: string | null | undefined): boolean {
...
export function normalizeSources(sources: string | string[] | null | undefined): string[] {
    if (!sources) return [];
...
function inferContentSource(chunk: string): ContentSource {
...
function inferOrigin(text: string, contentSource: ContentSource, sources: string[], opts: EvidenceGateOptions): ClaimOrigin {
...
function finalizeDecision(allowed: boolean, ctx: FinalizeContext): EvidenceGateResult {
...
export class EvidenceGate {
     ...
     *   Model + Assumptions + Equations + Numerical Method + Output
     */
    static simulationHeader(model: string | null | undefined, assumptions: string | string[] | null | undefined, equations: string | null | undefined, numericalMethod: string | null | undefined): string {
     ...
     */
    static gate(claim: string, opts: EvidenceGateOptions = {}): EvidenceGateResult {
     ...
     */
    static parseEvidence(content: string): { markers: EvidenceMarker[]; claims: EvidenceGateResult[] } {
        const text = String(content || '');
        const markers: EvidenceMarker[] = [];
        const claimBlocks = EvidenceGate.extractClaims(text);

        const markerPatterns: Array<{ type: string; regex: RegExp }> = [
     ...
     */
    static validateNote(noteContent: string): NoteValidationResult {
        const text = String(noteContent || '');
        const claimBlocks = EvidenceGate.extractClaims(text);
        const issues: Array<{ index: number; text: string; reason: string }> = [];
        const summary: { allowed: number; blocked: number; origins: Record<string, number>; trust: Record<string, number> } = { allowed: 0, blocked: 0, origins: {}, trust: {} };
     ...
     */
    static extractClaims(content: string): ClaimBlock[] {
        const text = String(content || '');
        if (!text.trim()) return [];
     ...
     */
    static formatClaim(claimText: string, result: EvidenceGateResult): string {
        const lines: string[] = [];
        lines.push('- ' + String(claimText || '').trim());
        const meta: string[] = (result && result.markers ? result.markers : []).filter(Boolean);
        ...
        if (result && result.extra && Array.isArray(result.extra.assumptions) && result.extra.assumptions.length > 0) {
            meta.push('[ASSUMPTION: ' + (result.extra.assumptions as string[]).join('; ') + ']');
        }
