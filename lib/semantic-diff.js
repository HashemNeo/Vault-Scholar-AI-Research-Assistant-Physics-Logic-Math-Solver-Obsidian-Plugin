 ...
 *   Description: ...
 */
function parseSemanticResult(raw) {
 ...
 */
function buildPrompt(previous, current) {
 ...
 *
 * @param {string} previous - Previous note content.
 * @param {string} current - Current note content.
 * @param {Function} generateFn - Async (prompt) => string, the LLM call.
 * @returns {Promise<Object>} { ok, result }
 */
async function semanticDiff(previous, current, generateFn) {
    try {
...
module.exports = { semanticDiff, parseSemanticResult, buildPrompt };